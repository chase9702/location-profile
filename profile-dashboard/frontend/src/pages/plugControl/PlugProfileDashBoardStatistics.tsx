...
const PlugProfileDashBoardStatistics = (): React.ReactElement => {
  ...
  const [selectDeviceModelData, setSelectDeviceModelData] =
    useState<string>('TOTAL');

  const [plugFirmwareVersionLoading, setPlugFirmwareVersionLoading] =
    useState(true);
  const [interpolationMonthlyLoading, setInterpolationMonthlyLoading] =
    useState(true);
  const [interpolationDailyLoading, setInterpolationDailyLoading] =
    useState(true);
  ...
  const firmwareVersionFetch = () => {
    ...
    get<[]>(
      `/api/plug/statistic/firmware-version-info?dvc_mdl=${selectDeviceModelData}`
    )
      .then((jsonData) => {
    ...
    <div>
      ...
      <Card style={{ padding: '10px' }}>
        ...
        <Tabs defaultActiveKey="1">
          <TabPane tab="모델별 보간트립 그래프" key="1">
            <Spin
              spinning={interpolationDailyLoading}
              indicator={<LoadingOutlined />}
              tip="로딩 중..."
            >
              <PlugInterpolationDailyChart
                interpolationDailyChartData={interpolationDailyData}
              />
            </Spin>
          </TabPane>
          <TabPane tab="모델별 보간트립 데이터" key="2">
            ...
            >
              <PlugInterpolationDailyTable
                interpolationDailyTableData={interpolationDailyData}
              />
            </Spin>
          </TabPane>
        </Tabs>
      ...
      <Card style={{ padding: '10px' }}>
        ...
        <Tabs defaultActiveKey="2">
          <TabPane tab="모델별 보간트립 그래프" key="1">
            <Spin
              spinning={interpolationMonthlyLoading}
              indicator={<LoadingOutlined />}
              tip="로딩 중..."
            >
              <PlugInterpolationMonthlyChart
                interpolationMonthlyChartData={interpolationMonthlyData}
              />
            </Spin>
          </TabPane>
          <TabPane tab="모델별 보간트립 데이터" key="2">
            ...
            >
              <PlugInterpolationMonthlyTable
                interpolationMonthlyTableData={interpolationMonthlyData}
              />
            </Spin>
          </TabPane>
        </Tabs>
      ...
      <Card style={{ padding: '10px' }}>
        ...
        <Tabs defaultActiveKey="2">
          <TabPane tab="모델별 Zero 트립 그래프" key="1">
            <Spin
              spinning={zeroGpsMonthlyLoading}
              indicator={<LoadingOutlined />}
              tip="로딩 중..."
            >
              <PlugZeroGpsMonthlyChart
                zeroGpsMonthlyChartData={zeroGpsMonthlyData}
              />
            </Spin>
          </TabPane>
          <TabPane tab="모델별 Zero 트립 데이터" key="2">
            ...
            >
              <PlugZeroGpsMonthlyTable
                zeroGpsMonthlyTableData={zeroGpsMonthlyData}
              />
            </Spin>
          </TabPane>
        </Tabs>
      ...
      <Card style={{ padding: '10px' }}>
        ...
        <Spin
          spinning={plugFirmwareVersionLoading}
          indicator={<LoadingOutlined />}
          tip="로딩 중..."
        >
          <PlugFirmwareVersion
            plugFirmwareVersionData={plugFirmwareVersionData}
          />
        </Spin>
      </Card>
    </div>
