...
const PlugZeroGpsDailyChart = (props: {
  zeroGpsDailyChartData: any[];
}): React.ReactElement => {
  const { zeroGpsDailyChartData } = props;
  const zeroGpsDailyGroupData = _.groupBy(
    zeroGpsDailyChartData,
  ...
  const zeroGpsDailyChartDataResult = _.map(zeroGpsDailyGroupData, (group) => {
    ...
    const sum_zero_trip_rt =
      sum_total_trip_cnt !== 0
        ? (sum_zero_trip_cnt / sum_total_trip_cnt) * 100
        : 0;
