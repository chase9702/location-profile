...
const PlugZeroGpsMonthlyChart = (props: {
  zeroGpsMonthlyChartData: any[];
}): React.ReactElement => {
  const { zeroGpsMonthlyChartData } = props;
  const zeroGpsMonthlyGroupData = _.groupBy(
    zeroGpsMonthlyChartData,
    (item) => `${item.dvc_mdl}-${item.bs_dt}`
  );

  const zeroGpsMonthlyChartDataResult = _.map(
    zeroGpsMonthlyGroupData,
    (group) => {
      const sumTotalCnt = _.sumBy(group, 'sum_total_trip_cnt');
      const sumZeroTripCnt = _.sumBy(group, 'sum_zero_trip_cnt');
      const sumZeroTripRt =
        sumTotalCnt !== 0 ? (sumZeroTripCnt / sumTotalCnt) * 100 : 0;
