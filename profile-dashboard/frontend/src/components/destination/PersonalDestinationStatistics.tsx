...
const CustomKeplerMap = lazy(
  () => import('@src/components/common/CustomKeplerMap')
);
  ...
  const personalDestinationFetch = (queryString: string) => {
    ...
    get<[]>(`/api/location/destination/personal/?${queryString}`)
      .then((jsonData) => {
        ...
        const h3FormattedData =
          'geometry,Address,Rank,Count\n' + h3FormatData(jsonData);
      ...
      <Row>
        <Col span={15}>
          <Suspense fallback={<div>Loading Map...</div>}>
            <CustomKeplerMap heightRatio={70} id={'personalMap'} />
          </Suspense>
        </Col>
        <Col span={9}>
          <Card>
            <Spin
              spinning={personalTableLoading}
              indicator={<LoadingOutlined />}
              tip="로딩 중..."
            >
              <Table
                columns={personalDestinationColumns}
                ...
                onRow={(record, rowIndex) => {
                  return {
                    onClick: (event) => {
                      ...
                      store.dispatch(
                        updateMap(extractFirstCoordinate(record.h3cell))
                      );
