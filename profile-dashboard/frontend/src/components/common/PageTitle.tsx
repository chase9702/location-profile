...
const descFormat = (description: string[]) => {
  return description
    .slice(1)
    .reduce(
      (acc, curr, index) => [...acc, <br key={index} />, '- ' + curr],
      ['- ' + description[0]]
    );
};
...
const PageTitle = (props: Props): React.ReactElement => {
  return (
    <Typography>
      ...
      {props.description && (
        <div className="page-description">{descFormat(props.description)}</div>
      )}
      {props.extraDescription && (
        <div className="page-extra-description">{props.extraDescription}</div>
      )}
    </Typography>
  );
};
