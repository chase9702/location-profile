...
const MonitoringDeviceChart = (props: Props): React.ReactElement => {
  ...
  const groupByAndSum = (
    ...
    const grouped: {
      [key: string]: { part_dt: string; metric: string; sum: number };
    } = {};
    ...
    const totalByPartDt: { [key: string]: number } = {};
    ...
    data.forEach((item) => {
      ...
      // 처음 나오는 total 값을 저장
      if (!totalByPartDt[item.part_dt]) {
        totalByPartDt[item.part_dt] = item.total;
      }
    });
    ...
    Object.keys(totalByPartDt).forEach((part_dt) => {
      result.push({
        part_dt,
        metric: '합계',
        sum: Math.round(totalByPartDt[part_dt] * 100) / 100,
      });
    });
    ...
    result.sort((a, b) => {
      ...
      if (a.part_dt > b.part_dt) return 1;
      if (a.metric === '합계') return -1;
      if (b.metric === '합계') return 1;
      return 0;
    });
  ...
  const handleDistanceSelectChange = (
    value: string,
    option:
      | { value: string; label: string }
      | {
