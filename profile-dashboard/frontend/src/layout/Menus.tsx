...
const Menus = (): React.ReactElement => {
  ...
  const storedUserRole = useSelector(
    (state: StoreState) => state.auth.userRole
  );
  const logout = () => {
    clearLocalStorage();
    dispatch(
  ...
  return (
    <div>
      <Menu theme="dark" mode="horizontal" className="cube-header-menus">
        <Menu.Item key="home">
          <HomeOutlined className="cube-main-icon" />
          <Link to="/" />
        </Menu.Item>
        {renderMenu()}
      </Menu>
